...
beforeEach(() => {
  ...
  handler = require('../../../../lambdas/assistanceRequestWebsocket/connectionAuthorizer/handler')(
    dependencies
  );
});
...
test('Should return unauthorized when the user has a role other than Control Room Operator and Administrator', async () => {
  const event = {
    queryStringParameters: { Authorization: validJwt },
    methodArn: validMethodArn,
  };
