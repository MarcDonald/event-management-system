...
module.exports = (dependencies) => async (event) => {
  ...
  const { eventId, positionId } = event.pathParameters;
  ...
  if (!eventId || !positionId) {
    return {
      ...
      statusCode: 400,
      body: JSON.stringify({
        message: 'Event ID and Position ID must be provided',
      }),
    };
  }
  ...
  try {
    ...
    const formattedAssistanceRequests = result.Items.filter(
      (dbItem) => dbItem.position.positionId === positionId
    ).map((dbItem) => {
    ...
    return {
      ...response,
      body: JSON.stringify({
        message: `Error getting assistance requests for position '${positionId}' on event '${eventId}' - ${e.message}`,
      }),
    };
  }
};
